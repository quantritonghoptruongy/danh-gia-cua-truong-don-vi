...
const SHEET_NAME       = (import.meta.env.VITE_SHEET_NAME as string) || 'ATCS';
...
// ─────────────────────────────────────────────────────────────────────────────
// GOOGLE SHEETS API — thay thế hoàn toàn google.script.run
// ─────────────────────────────────────────────────────────────────────────────

/** Đọc toàn bộ dữ liệu nhân viên từ sheet (tương đương getEmployeeData trong Code.gs) */
async function apiFetchEmployees(accessToken: string): Promise<Employee[]> {
  const range = encodeURIComponent(`${SHEET_NAME}!A:X`);
  const url   = `${SHEETS_BASE}/${SPREADSHEET_ID}/values/${range}?valueRenderOption=FORMATTED_VALUE`;
  const res   = await fetch(url, { headers: { Authorization: `Bearer ${accessToken}` } });

  if (res.status === 401) throw new Error('TOKEN_EXPIRED');
  if (!res.ok) {
    const err = await res.json().catch(() => ({})) as { error?: { message?: string } };
    throw new Error(err.error?.message || 'Không đọc được dữ liệu từ Google Sheets');
  }

  const json = await res.json() as { values?: string[][] };
  const rows: string[][] = json.values || [];

  // 3 dòng đầu là header — dữ liệu từ dòng 4 (index 3), giống Code.gs
  ...
  for (let i = 3; i < rows.length; i++) {
    ...
    results.push({
      ...
      rowNumber: i + 1,           // 1-indexed cho Sheets API
      stt:       r[0]  || '',
...
/** Lưu kết quả đánh giá vào cột V, W, X (tương đương updateEvaluation trong Code.gs) */
async function apiSaveEvaluation(
  accessToken: string,
  payload: Employee,
  userEmail: string,
): Promise<void> {
  // Kiểm tra khóa thời gian (mùng 3 tháng kế tiếp) — giống Code.gs
  if (payload.date) {
  ...
  // Ghi vết giống hệt Code.gs: "email (dd/mm/yyyy hh:mm:ss)"
  const auditLog = `${userEmail} (${timestamp})`;

  // Cột V=22, W=23, X=24 → range V{row}:X{row} — giống sheet.getRange(row, 22, 1, 3)
  const range = `${SHEET_NAME}!V${payload.rowNumber}:X${payload.rowNumber}`;
...
/** Lấy thông tin tài khoản Google từ access token */
...
// ─────────────────────────────────────────────────────────────────────────────
// ICON (giữ nguyên từ eval_app_js.pdf)
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// COMPONENT: NHẬP ĐIỂM SỐ (Nhóm II, III, IV) — giữ nguyên từ eval_app_js
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// COMPONENT: TIÊU CHÍ ĐẠO ĐỨC (Nhóm I) — giữ nguyên từ eval_app_js
// ─────────────────────────────────────────────────────────────────────────────
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 bg-gradient-to-br from-emerald-50 to-slate-100">
      <div className="bg-white p-8 rounded-2xl shadow-xl flex flex-col items-center max-w-sm w-full text-center border-t-4 border-emerald-600">
        ...
        <h1 className="text-2xl font-black text-slate-800 tracking-tighter">Trung tâm ATCS</h1>
        <p className="text-emerald-700 font-bold text-sm mt-1">Hệ thống Đánh giá Nhân viên</p>
        <p className="text-slate-400 text-xs mt-2 mb-6">
          Đăng nhập để thực hiện đánh giá.<br />Tài khoản sẽ được ghi vết tự động khi lưu.
        </p>

        {missingConfig ? (
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 text-sm text-amber-800 text-left w-full space-y-1">
            <p className="font-bold">⚠️ Chưa cấu hình đủ biến môi trường!</p>
            <p className="text-xs mt-1">Tạo file <code className="bg-amber-100 px-1 rounded">.env</code>:</p>
            <pre className="text-xs bg-amber-100 rounded p-2 mt-1 whitespace-pre-wrap">
{`VITE_GOOGLE_CLIENT_ID=xxx.apps.googleusercontent.com
VITE_SPREADSHEET_ID=1BxiM...your_sheet_id
VITE_SHEET_NAME=ATCS`}
            </pre>
          </div>
        ) : (
          <>
            <button onClick={onLogin} disabled={isLoggingIn}
              ...
              {isLoggingIn ? 'Đang xử lý...' : 'Đăng nhập bằng Google'}
            </button>
            <p className="text-[10px] text-slate-400 mt-4 leading-relaxed">
              Yêu cầu quyền đọc/ghi Google Sheets.<br />
              Chỉ dùng để ghi kết quả đánh giá vào sheet của đơn vị.
            </p>
          </>
        )}
      </div>
...
// ─────────────────────────────────────────────────────────────────────────────
// COMPONENT: ỨNG DỤNG ĐÁNH GIÁ CHÍNH — toàn bộ UI từ eval_app_js
// ─────────────────────────────────────────────────────────────────────────────
interface MainAppProps { user: GoogleUser; accessToken: string; onLogout: () => void; }
const MainApp = ({ user, accessToken, onLogout }: MainAppProps) => {
  ...
  const loadData = useCallback(async () => {
    ...
    setLoadError('');
    try {
      const data = await apiFetchEmployees(accessToken);
      setEmployees(data);
    } catch (e) {
  ...
  // ── Kiểm tra khóa chỉnh sửa — giống hệt Code.gs ─────────────────────────
  ...
  // ── Cập nhật điểm (local state, không lưu lên sheet) ─────────────────────
  ...
  const handleSave = async () => {
    if (isReadOnly || !selectedEmp) return;
    if (!selectedEmp.status) {
      alert('Vui lòng chọn xếp loại trước khi lưu!');
      return;
    }
    ...
    try {
      ...
      setEmployees(prev => prev.map(e =>
        e.rowNumber === selectedEmp.rowNumber
          ? { ...selectedEmp, evaluated: true }
          : e,
      ));
      setSaveSuccess(true);
  ...
  if (loading) return (
    <div className="min-h-screen flex items-center justify-center bg-slate-50">
      <div className="text-center">
        ...
        <p className="text-slate-400 font-black text-[10px] tracking-widest uppercase italic">Đang đồng bộ dữ liệu...</p>
      </div>
    </div>
  );
    ...
    <div className="min-h-screen flex items-center justify-center bg-slate-50 p-6">
      <div className="bg-white rounded-2xl p-8 shadow-xl max-w-sm w-full text-center space-y-4">
        ...
        <p className="text-xs text-slate-400">
          Kiểm tra <code className="bg-slate-100 px-1 rounded">VITE_SPREADSHEET_ID</code> và đảm bảo tài khoản có quyền truy cập Google Sheet.
        </p>
        <div className="flex gap-2">
          <button onClick={loadData} className="flex-1 bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-3 rounded-xl text-sm">
            Thử lại
          </button>
          <button onClick={onLogout} className="flex-1 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold py-3 rounded-xl text-sm">
            Đăng xuất
          </button>
        </div>
      </div>
      ...
      <header className="bg-emerald-700 text-white p-4 sticky top-0 z-20 shadow-md">
        <div className="max-w-4xl mx-auto flex justify-between items-center">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-black italic tracking-tighter leading-none">Trung tâm ATCS</h1>
              <p className="text-[10px] font-bold opacity-80 uppercase tracking-widest mt-1">
                {activeTab === 'current' ? `Tháng hiện tại: ${currentPeriod}` : `Lịch sử: Tháng ${historyPeriod}`}
              </p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <button onClick={loadData} title="Tải lại dữ liệu"
            ...
            <div className="p-5 sm:p-6 pt-8 overflow-y-auto">
              ...
              <div className="space-y-4 mb-8 bg-emerald-50/50 p-4 rounded-2xl border border-emerald-100">
                ...
                <div>
                  <label className="block text-[10px] font-black text-emerald-700 uppercase tracking-widest mb-2 ml-1">
                    Ghi chú & Nhận xét
                    {selectedEmp.status === 'A+' && !isReadOnly && (
                      <span className="text-red-500 ml-1 text-sm">* (Bắt buộc)</span>
                    )}
                  </label>
                  <textarea disabled={isReadOnly}
                    ...
                    placeholder={selectedEmp.status === 'A+' && !isReadOnly
                      ? 'Vui lòng nhập lý do/thành tích để giải trình cho mốc Xuất sắc (A+)...'
                      : 'Ý kiến chỉ đạo...'}
                ...
                {saveSuccess && (
                  <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-3 text-sm text-emerald-700 font-bold">
                    ✅ Đã lưu & ghi vết email <strong>{user.email}</strong> vào Google Sheets!
                  </div>
                )}
              </div>
              ...
              <div className="border-t border-slate-200 pt-6 mt-2">
                ...
                <div className="mb-8">
                  <h3 className="text-xs font-black text-rose-700 uppercase tracking-widest mb-3 ml-2">
                    I. Phẩm chất đạo đức (Tiêu chí bắt buộc)
                  </h3>
                ...
                <div className="mb-8">
                  <h3 className="text-xs font-black text-blue-700 uppercase tracking-widest mb-3 ml-2">
                    II. Tinh thần trách nhiệm, thái độ phục vụ (15đ)
                  </h3>
                  <div className="flex flex-col gap-3 bg-slate-50 p-2 sm:p-3 rounded-3xl border border-slate-100">
                ...
                <div className="mb-8">
                  <h3 className="text-xs font-black text-amber-600 uppercase tracking-widest mb-3 ml-2">
                    III. Ý thức kỷ luật, kỷ cương (15đ)
                  </h3>
                  <div className="flex flex-col gap-3 bg-slate-50 p-2 sm:p-3 rounded-3xl border border-slate-100">
                ...
                <div className="mb-8">
                  <h3 className="text-xs font-black text-emerald-700 uppercase tracking-widest mb-3 ml-2">
                    IV. Khả năng đáp ứng yêu cầu (70đ)
                  </h3>
                  <div className="flex flex-col gap-3 bg-slate-50 p-2 sm:p-3 rounded-3xl border border-slate-100">
                    <DetailedScoreInput disabled={isReadOnly} label="Chỉ số 4.1" colorClass="bg-emerald-100 text-emerald-700" title="Khả năng đáp ứng yêu cầu của cấp quản lý có thẩm quyền về số lượng, tiến độ." max={20} value={selectedEmp.c41} onChange={v => updateScore('c41', v)} />
              ...
              {isReadOnly ? (
                <button disabled className="w-full bg-slate-200 text-slate-400 py-5 rounded-2xl font-black flex items-center justify-center gap-3 mb-4 cursor-not-allowed shadow-none">
...
// ─────────────────────────────────────────────────────────────────────────────
// AUTH WRAPPER — dùng useGoogleLogin (không phải GoogleLogin button)
// để lấy access_token có scope đọc/ghi Google Sheets
// ─────────────────────────────────────────────────────────────────────────────
const AuthWrapper = () => {
  ...
  const handleLogout = useCallback(() => { setUser(null); setAccessToken(''); }, []);
  ...
  const login = useGoogleLogin({
    ...
    // ⚠️ Scope quan trọng: spreadsheets để đọc/ghi Google Sheets
  ...
  return <MainApp user={user} accessToken={accessToken} onLogout={handleLogout} />;
};
